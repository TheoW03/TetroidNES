...
{
    return (uint16_t)addressBus.read_8bit(cpu.PC);
}
uint16_t Zero_page_AddressMode_X(CPUProcessor cpu)
{
    return (addressBus.read_8bit(cpu.PC) + cpu.X_Reg);
}
uint16_t Zero_page_AddressMode_Y(CPUProcessor cpu)
{
    return ((addressBus.read_8bit(cpu.PC) + cpu.Y_Reg));
}

uint16_t Absolute(CPUProcessor cpu)
{
    // printf("%x \n", read_16bit(PC));
    return (addressBus.read_16bit(cpu.PC));
}

uint16_t Absolute_page_AddressMode_X(CPUProcessor cpu)
{
    return (addressBus.read_16bit(cpu.PC) + cpu.X_Reg);
}
...
{
    uint16_t value = (uint16_t)addressBus.read_8bit(cpu.PC);
    return addressBus.read_16bit(value);
}
uint16_t Indirect_AddressModeX(CPUProcessor cpu)
{
    return addressBus.read_16bit((addressBus.read_8bit(cpu.PC) + cpu.X_Reg));
}
uint16_t Indirect_AddressModeY(CPUProcessor cpu)
{
    return (addressBus.read_16bit(addressBus.read_8bit(cpu.PC)) + cpu.Y_Reg);
}