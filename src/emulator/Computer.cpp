...
void run()
{
	...
	while (1)
	{
			...
			|| current_instruction == 0xA1 || current_instruction == 0xB1)
		{
			...
			set_zero(cpu_Processor.A_Reg, cpu_Processor);
			set_negative(cpu_Processor.A_Reg, cpu_Processor);
		}
				 ...
				 || current_instruction == 0xAE || current_instruction == 0xBE)
		{
			...
			set_zero(cpu_Processor.X_Reg, cpu_Processor);
			set_negative(cpu_Processor.X_Reg, cpu_Processor);
		}
				 ...
				 || current_instruction == 0xBC)							   //
		{
			...
			set_zero(cpu_Processor.Y_Reg, cpu_Processor);
			set_negative(cpu_Processor.Y_Reg, cpu_Processor);
		}
